 ...
 *  N4CT: Network Act, an example client of the VEI library
...
test_one_packet()
{
  ...
  {
    fprintf(stdout, 
	    "[nact] injection request failed with code %d\n", 
	    injection_result);
  }else{
    fprintf(stdout, 
	    "[nact] buffered %d packets\n", 
	    injection_result);
  }
  return;
}
...
do_injection_script(void* arg)
{
  ...
  fprintf(stdout,
	  "\n[i_thread] calling test_one_packet()...\n");
  fflush(stdout);
	...
	char* dst)
{
  ...
  if(0==init_result)
  {
    fprintf(stdout, "[nact] initialized OK\n");
  }else{
  ...
  {
    fprintf(stderr,
	    "[nact] failed to create actor thread: %s\n",
	    strerror(err));
    exit(-40);
  }else{
    fprintf(stdout,
	    "[nact] created actor thread\n");
    fflush(stdout);
  }

  fprintf(stdout,
	  "[nact] invoking start_vei_transcription...\n");
...
/**
 * Network Act (nact)
 * nact src.cap dst.cap [script.njs]
 * 
 * - open src (must exist)
 * - open and create dst
 * - read script.njs (if supplied) for instructions on what to
 *   inject when
 */
int main(int argc,
	 char* argv[])
{
  if(3==argc)
  {
    ...
    fprintf(stdout,
	    "[nact] calling shutdown()...\n");
    fflush(stdout);
    shutdown();

  }else{
    fprintf(stderr,
	    "nact srcfile.cap dstfile.cap [script.njs]\n");
    return -1;
  }
